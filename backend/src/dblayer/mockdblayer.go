...
//NewMockDBLayerWithData - contructor with data
...
func (mock *MockDBLayer) SignInUser(email, password string) (models.Customer, error) {
	...
	for _, customer := range mock.customers {
		if strings.EqualFold(customer.Email, email) && customer.Pass == password {
...
func (mock *MockDBLayer) SignOutUserByID(id int) error {
	...
	return fmt.Errorf("Could not sign out user with id %d", id)
}
...
func (mock *MockDBLayer) AddOrder(order models.Order) error {
	...
	return fmt.Errorf("Could not find customer with id %d for order", order.CustomerID)
}

//GetCreditCardCID - get credit card ID
...
//SaveCreditCardForCustomer - save credit card for customer
